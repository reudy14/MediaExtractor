...
parser.add_argument("-i", "--InputPath", help="Path to start at", type= pathlib.Path, required=True)
parser.add_argument("-o", "--OutputPath", help="Path to output at", type= pathlib.Path)
parser.add_argument("--no-recursion", dest="Recursion", help="Should Script Run through sub Direcory's", action=argparse.BooleanOptionalAction, default=True)
parser.add_argument("--FileTypes", help="The File Type/Types you want to Extract. Deliminate multiple with ','", default=mediaEndings)
...
def getMedia(path, media = None):
    ...
    desiredDir = [pathlib.Path(os.path.join(path, mediaName)) for mediaName in desiredDir]
...
i = 1
for m in media:
    dest_dir = os.path.join(outputPath, m.name)
